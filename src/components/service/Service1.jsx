      ...
      let tHero = gsap.context(() => {
        if (device_width > 1200) {
          ...
          window.addEventListener("scroll", () => {
            const serviceItems = document.querySelectorAll(".service__item-6");
            serviceItems.forEach((item) => {
              const positionTop = item.getBoundingClientRect().top;
              if (
                positionTop <= window.pageYOffset &&
                positionTop + item.offsetHeight > window.pageYOffset
              ) {
                const secId = item.dataset.secid;
                const serviceListItems = document.querySelectorAll(
                  ".service__list-6 li"
                );
                serviceListItems[secId - 1].classList.add("active");
                serviceListItems.forEach((li, index) => {
                  if (index !== secId - 1) {
